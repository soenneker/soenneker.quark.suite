...
public sealed class QuarkSortablePlaywrightTests : QuarkPlaywrightTest
{
    ...
    public async ValueTask Sortable_examples_reorder_basic_and_handle_only_lists_while_disabled_list_stays_inert()
    {
        ...
        await WaitForSortableReady(page, basicList);
        ...
        await WaitForSortableReady(page, handleList);
        ...
        await DragSortableItemToTarget(page, announceRow, promoteRow);
        await Assertions.Expect(handleState).ToContainTextAsync("No changes yet.");
        ...
        await DragSortableItemToTarget(page, announceHandle, promoteRow);

        await Assertions.Expect(handleState).ToContainTextAsync("announce moved from 4 to 3.");
        (await GetSortableItemOrder(handleList)).Should().Equal(["build", "smoke", "announce", "promote"]);
        ...
        await WaitForSortableReady(page, disabledList);

        await Assertions.Expect(disabledList).ToHaveAttributeAsync("aria-disabled", "true");
        (await GetSortableItemOrder(disabledList)).Should().Equal(["contract", "dns", "handoff"]);
        ...
        await DragSortableItemToTarget(page, handoff, contract);

        (await GetSortableItemOrder(disabledList)).Should().Equal(["contract", "dns", "handoff"]);
        consoleErrors.Should().BeEmpty();
        pageErrors.Should().BeEmpty();
    }
    private static async Task DragSortableItemToTarget(IPage page, ILocator source, ILocator target)
    {
    ...
    private static async Task<string[]> GetSortableItemOrder(ILocator list)
    {
    ...
    private static async Task WaitForSortableReady(IPage page, ILocator list)
    {
