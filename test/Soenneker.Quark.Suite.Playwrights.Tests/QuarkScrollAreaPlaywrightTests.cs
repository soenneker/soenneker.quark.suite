...
[Test]
    public async ValueTask Scroll_area_examples_scroll_their_viewports_in_vertical_horizontal_and_rtl_layouts()
    {
        ...
        var verticalSection = page.Locator("section").Filter(new LocatorFilterOptions { HasText = "Fixed-height tag list with separators between items." }).First;
        var horizontalSection = page.Locator("section").Filter(new LocatorFilterOptions { HasText = "Horizontal scroll areas help showcase collections without forcing every item into a narrow grid." }).First;
        var rtlSection = page.Locator("section").Filter(new LocatorFilterOptions { HasText = "Scroll areas can also present content naturally in right-to-left layouts." }).First;
