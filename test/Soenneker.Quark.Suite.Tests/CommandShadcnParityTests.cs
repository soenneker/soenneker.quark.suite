...
public sealed partial class RenderedShadcnParityTests
{
    ...
    public void Command_slots_match_shadcn_base_classes()
    {
        ...
        commandClasses.Should().NotContain("p-1");
        ...
        wrapperClasses.Should().NotContain("p-1");
        wrapperClasses.Should().NotContain("pb-0");

        cut.FindAll("[data-slot='input-group']").Should().BeEmpty();
        ...
        itemClasses.Should().NotContain("data-[selected=true]:bg-muted");
        itemClasses.Should().NotContain("data-[selected=true]:text-foreground");
        itemClasses.Should().Contain("[&_svg]:pointer-events-none");
        itemClasses.Should().Contain("[&_svg:not([class*='text-'])]:text-muted-foreground");
        itemClasses.Should().NotContain("data-selected:*:[svg]:text-foreground");
    ...
    public void Command_source_matches_shadcn_v4_contract()
    {
        ...
        item.Should().NotContain("group/command-item");
        ...
        dialog.Should().Contain("**:data-[slot=command-input-wrapper]:h-12");
        dialog.Should().Contain("[&_[cmdk-group]:not([hidden])_~[cmdk-group]]:pt-0");
