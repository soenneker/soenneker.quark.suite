...
public sealed partial class RenderedShadcnParityTests
{
    ...
    public void Typography_headings_and_supporting_text_match_shadcn_examples()
    {
        ...
        h1.Find("[data-slot='h1']").GetAttribute("class")!.Should().ContainAll("scroll-m-20", "text-center", "text-4xl", "font-extrabold", "tracking-tight", "text-balance");
        h1.Find("[data-slot='h1']").GetAttribute("class")!.Should().NotContain("lg:text-5xl");
        h2.Find("[data-slot='h2']").GetAttribute("class")!.Should().ContainAll("mt-10", "scroll-m-20", "border-b", "pb-2", "text-3xl", "font-semibold", "tracking-tight", "transition-colors", "first:mt-0");
        h3.Find("[data-slot='h3']").GetAttribute("class")!.Should().ContainAll("mt-8", "scroll-m-20", "text-2xl", "font-semibold", "tracking-tight");
        h4.Find("[data-slot='h4']").GetAttribute("class")!.Should().ContainAll("scroll-m-20", "text-xl", "font-semibold", "tracking-tight");
        lead.Find("[data-slot='lead']").GetAttribute("class")!.Should().ContainAll("text-xl", "leading-7", "text-muted-foreground");
    ...
    [Test]
    public void Blockquote_uses_logical_start_border_for_rtl_parity()
    {
        var blockquote = Render<Blockquote>(parameters => parameters.Add(p => p.ChildContent, "Quote"));
        var classes = blockquote.Find("[data-slot='blockquote']").GetAttribute("class")!;

        classes.Should().Contain("mt-6");
        classes.Should().Contain("border-s-2");
        classes.Should().Contain("ps-6");
        classes.Should().Contain("italic");
