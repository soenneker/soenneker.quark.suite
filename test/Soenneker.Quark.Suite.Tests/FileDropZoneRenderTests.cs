...
public sealed partial class RenderedShadcnParityTests
{
    ...
    public async Task FileDropZone_failed_upload_can_retry_after_another_selection()
    {
        ...
        var wrapper = cut.FindComponent<FileDrop>();
        wrapper.Instance.RenderInput.Should().BeFalse();
        var firstInput = cut.FindComponent<InputFile>();
        wrapper.Instance.InputId.Should().Be(firstInput.Find("input").Id);
        firstInput.UploadFiles(InputFileContent.CreateFromText("one", "one.pdf"));
        ...
        cut.FindComponents<InputFile>().First().Instance.Should().BeSameAs(firstInput.Instance);
        wrapper.Instance.InputId.Should().Be(cut.FindComponents<InputFile>().Last().Find("input").Id);
